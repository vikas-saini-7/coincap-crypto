...
const HeroCoins = () => {
    ...
    return (
        <div className='flex gap-4 mt-[8%] items-center justify-center'>
            {loading && <p>Loading..</p>}
            {!loading && coinsData?.slice(0,3).map((item) => (
                <div className='flex items-center gap-6 px-[5%] py-[3%] border border-gray-500 border-opacity-20 bg-opacity-10 rounded'>
                    ...
                    <div>
                        ...
                        <h1 className='flex items-center gap-1 text-gray-200'>  <span className={`${item.price_change_percentage_24h >= 0 ? 'text-green-500' : 'text-red-700'}`}> ₹ {item.current_price}</span></h1>
                    </div>
                </div>
            ))}
        </div>
